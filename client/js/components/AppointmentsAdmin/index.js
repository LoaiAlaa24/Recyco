...
export default class AppointmentsAdmin extends React.Component {
...
async componentDidMount(){
  await axios({
    method:'POST',
    url:'https://recyco.herokuapp.com/api/appointments/viewall'
  }).then(res=>{
    var myAppointments=[]
  ...
  render() {
    ...
    for(var i = 0; i < this.state.appointments.length; i++){
      ...
      if(this.state.appointments[index].status === "PENDING")
        buttonComponent.push(<Button title="In progress..." color="green" onPress={()=>{this.handleClaim(index)}}/>)
